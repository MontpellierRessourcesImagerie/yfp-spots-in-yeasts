...
class TestImagesFinder(unittest.TestCase):
    ...
    def test_number_images(self):
        with patch("os.listdir") as ldir, patch("os.path.isfile") as is_file, patch("os.path.isdir") as idir:
            ldir.return_value = self.folder_content
            is_file.return_value = True
            idir.return_value = True
            ...
            image_sets = seek_channels(input_dir, components)

            self.assertEqual(len(image_sets), 32, "Not all images were found.")
            for group in image_sets:
                # The "+ 4" comes from the fact that some elements are automatically added into the collection when initializing the `item` variable into `seek_channels`.
                self.assertEqual(len(components)+4, len(group), "Some group is missing a component, or has one in excess.")
